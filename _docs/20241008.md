...
本講義では、開発ボードとして、**Arduino UNO (R3 / R4)** もしくは **Raspberry Pi Pico W** （以下、RPi Pico W と省略） を標準のものとして進める予定です。
...
本講義内で、**単に「Arduino」というときは、開発ボードとしての Arduino Uno (R3 / R4) を指すものとします**が、開発ボードとしての Arduino も実際には下記のリンクにあるように、複数のバリエーション・バージョンが存在します。

https://docs.arduino.cc/hardware/

また、前述のように、Arduinoプロジェクトはソフトウェア・ハードウェア共に基本的にオープンソースとなっているため、ソースコードや回路図、基板設計図などが公開されており、Arduino ボードに関してはほぼ同じ仕様で開発元・製造元が異なる「互換品」も非常に多数存在しています。

近年では、Arduino プロジェクトのうちソフトウェアライブラリの部分を切り分けて **Arduino Core** という呼び方をするようになり、コミュニティの開発者によって、標準の Arduino ボード以外の開発ボードでも Arduino IDE を含むエコシステムを使って開発できるよう移植することが積極的に行われるようになりました。

これらは「互換品」とはまた別に、ハードウェアの構成としては違うもののピンの配置だけ合わせたものから、ピンの配置までもまたく違うもののまで多様です。

### Raspberry Pi Pico W について
...
このプロジェクトが当初開発・販売していた（今もし続けている）製品が **Raspberry Pi** という、Linux OS が動く小型のコンピューターです。

当初は教育用途の安価なコンピューターとして普及させることを目指していたため、一般的なデスクトップPCやラップトップPCと比べて価格が安く、小型であることが特徴でした。

また、ディスプレイモニタやキーボードなどの周辺機器を除いたコンピューターとしての基本的な機能がひとつの基板上にまとまっているため、現在では「シングルボードコンピューター（SBC）」の代表的なものとしても認知されています。

#### Raspberry Pi Pico (W / WH)

**Raspberry Pi Pico** は Raspberry Pi のプロジェクトに比較的最近追加された製品であり、それまでのような Linux OS が動く汎用的なSBCではなく、Arduino ボードのようにマイクロコントローラーが搭載された開発ボードです。
...
### Arduino UNO と Raspberry Pi Pico W のどちらかを選択するにあたって

下記はいずれも完全な説明ではありませんが、本講義内で扱う限りで知っておいた方が良さそうな両者の違いを挙げたものになります。
...
- Arduino UNO (R3 / R4)
  - 正規の販売元から購入すると永久保証を受けられる場合があります。
    - https://www.switch-science.com/pages/arduino-uno-permanent-warranty
- RPi Pico W
  - ある程度の期間の供給予定が製造元・販売元から明言されています（RPi Pico W は2031年まで予定されている）。

#### 開発のしやすさ、扱いやすさ

- Arduino UNO (R3 / R4)
  - Arduino IDE 標準として扱えるボードです。
  - USBコネクタはR3が **USB Type-A**、R4は **USB Type-C**
  - 「シールド」と呼ばれるハードウェアとしての機能を拡張する、一定の規格に沿った基板が数多く製造・販売さられています。
    - https://www.switch-science.com/search?q=arduino+uno+shield
    - https://www.sparkfun.com/categories/240
    - https://www.adafruit.com/category/21
- RPi Pico W
  - ピンの配置が標準のボードと異なります。
    - 同時に数が多いため、より多くのものを制御しやすいです。
  - Arduino IDE にボードの設定を追加することで、標準の Arduino ボードとほぼ同じように使えます。
  - USBコネクタは **Micro USB Type-B**
...
- Arduino UNO (R3 / R4)
  - 無線通信の機能を使うためには、より高価な Arduino UNO R4 WiFi を選択するか、シールドなど外付けのモジュールを使用する必要があります。
